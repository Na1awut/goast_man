...
>
> ทำอะไรไปแล้วและจะทำอะไรต่อ ดู [`PLAN.md`](PLAN.md)
...
| PromptPay | **ระบบพร้อม · รอตั้งค่า** | QR จริง + ตรวจสลิปกับ SlipOK ผ่าน Edge Function `verify-slip` (deploy แล้ว) · เว็บจริงยังซ่อนไว้จนกว่าจะใส่ SlipOK secret และ `PUBLIC_PROMPTPAY_API_URL` + `PUBLIC_PROMPTPAY_ID` (ขั้นตอนใน `supabase/README.md`) · โหมดเดโมยังใช้ QR จำลอง |
...
| 7 | `20260930000000_promptpay_slips.sql` | PromptPay: สถานะจ่ายเงิน, กันสลิปซ้ำ, ออเดอร์ที่ยังไม่จ่ายไม่ขึ้นให้คนหิ้ว, `rider_payouts_due()` / `mark_payout_paid()` |
| 8 | `seed.sql` | ร้านและเมนู |
...
ย้ายไปอยู่ใน [`PLAN.md`](PLAN.md) ที่เดียว: สิ่งที่ทำไปแล้ว, การตัดสินใจที่ทำไปแล้ว, อัปเดตล่าสุด, แผนระยะที่ 1-4 (พร้อมขนาดงานและเงื่อนไขว่าเสร็จเมื่อไร), เรื่องที่รอตัดสินใจ และความเสี่ยง

เรื่องเร่งที่สุดตอนนี้: สร้าง Supabase project จริงแล้วให้ทีมทดลองใช้ (ระยะที่ 1), ตัดสินใจเรื่องค่ากล่อง 5 บาท, ทางเข้าโหมดคนหิ้ว และเกณฑ์ verify คนหิ้ว
