...
<!-- ================================================================== -->
{#if toastMessage}
	<div class="fixed top-3 left-4 right-4 max-w-md mx-auto z-[60] bg-slate-900/95 backdrop-blur-md text-white px-4 py-3 rounded-2xl shadow-2xl text-xs font-medium flex items-center justify-between border border-[#F59E0B]/30">
			...
			<div class="space-y-2 max-h-64 overflow-y-auto">
				{#each DROPOFF_POINTS as loc}
					<button onclick={() => { currentDropoff = loc.name; checkoutDropoff = loc.name; showLocationModal = false; showToast(`📍 อัปเดตจุดรับเป็น ${loc.name}`); }}
						class="w-full text-left p-3 rounded-2xl border border-slate-100 hover:border-[#F59E0B]/40 hover:bg-[#FFFBEB] flex items-center justify-between transition-all active:scale-98">
						<div>
						...
						{#if currentDropoff === loc.name}
							<span class="text-[#F59E0B] font-bold text-[10px] bg-[#FFFBEB] px-2 py-0.5 rounded-full">✓ เลือก</span>
						{/if}
					</button>
				{/each}
			</div>
...
{#if showPromptPayModal}
	<div class="fixed inset-0 bg-slate-950/70 backdrop-blur-md z-50 flex justify-center items-center p-4">
		<div class="bg-white w-full max-w-sm rounded-[32px] p-6 shadow-2xl space-y-4 text-center relative">
			...
			<div class="bg-amber-50 text-[#F59E0B] text-xs font-bold px-3 py-1.5 rounded-full inline-block">⏱️ กรุณาชำระภายใน 10:00 นาที</div>
			...
			<div class="text-3xl font-black text-[#F59E0B]">{netTotal.toFixed(2)} ฿</div>
			<button onclick={() => { completeOrder(); }} class="w-full py-3.5 bg-[#F59E0B] text-white font-extrabold rounded-full text-sm shadow-lg shadow-[#F59E0B]/30 active:scale-95">
...
<!-- ================================================================== -->
{#if screen === 'LOGIN'}
	<div class="flex-1 flex flex-col items-center justify-center p-8 bg-gradient-to-b from-[#F59E0B] via-amber-500 to-amber-500 min-h-screen text-white text-center">
		...
		<p class="text-amber-100 text-sm font-medium mb-1">กูสแมน (ห่านบางมด 🪿)</p>
...
{:else}
	...
	<header class="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-slate-100 px-4 py-2.5 flex items-center justify-between shadow-sm">
		<button onclick={() => { screen = 'HOME'; screenHistory = []; }} class="flex items-center gap-2 group">
			<div class="w-9 h-9 rounded-2xl bg-gradient-to-tr from-[#F59E0B] to-amber-400 text-white flex items-center justify-center font-black text-xl shadow-md shadow-[#F59E0B]/20 group-hover:scale-105 transition-transform">🪿</div>
			<div>
				<div class="font-extrabold text-slate-900 text-sm leading-none">Goose Man <span class="text-[10px] bg-amber-100 text-[#F59E0B] px-1.5 py-0.5 rounded-full font-bold">ห่านบางมด 🪿</span></div>
				<div class="text-[10px] text-slate-400">KMUTT Campus P2P</div>
			</div>
		</button>
		<div class="flex items-center gap-2">
			<button onclick={() => (showLocationModal = true)} class="text-[10px] font-bold text-[#F59E0B] bg-[#FFFBEB] px-2.5 py-1 rounded-full border border-[#F59E0B]/20 truncate max-w-[140px]">📍 {currentDropoff.split('(')[0].trim()}</button>
			<button onclick={() => showToast('🔔 ไม่มีการแจ้งเตือนใหม่')} class="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-sm relative">
				🔔
				{#if activeOrders.length > 0}<span class="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-[#F59E0B] border-2 border-white"></span>{/if}
			</button>
		</div>
	</header>
	...
	<div class="flex-1 overflow-y-auto overscroll-contain pb-20">
		...
		{#if screen === 'HOME'}
			<div class="p-5 space-y-5">
				...
				<div class="flex items-center gap-3">
					<div class="w-12 h-12 rounded-2xl bg-[#FFFBEB] border-2 border-[#F59E0B]/30 flex items-center justify-center text-2xl shadow-sm">🦢</div>
					<div>
				...
				<!-- Location Chip -->
				<button onclick={() => (showLocationModal = true)} class="w-full bg-[#FFFBEB] text-[#F59E0B] p-3 rounded-2xl text-xs font-bold border border-[#F59E0B]/20 flex items-center justify-between shadow-sm">
					<span class="truncate">📍 จุดรับของ: <strong class="text-slate-900">{currentDropoff}</strong></span>
					<span class="text-[#F59E0B] font-bold whitespace-nowrap ml-2">เปลี่ยน ▾</span>
				</button>

				<!-- Hero Banner -->
				<div class="relative bg-gradient-to-r from-[#F59E0B] via-amber-500 to-amber-500 rounded-3xl p-5 text-white shadow-xl shadow-[#F59E0B]/20 overflow-hidden">
					...
					<p class="text-xs text-amber-100 mt-1">ค่าหิ้วเริ่มต้น 15.- ส่งตรงถึงตึกเรียน</p>
					<div class="flex gap-2 mt-3">
						<button onclick={() => goTo('ORDER_CREATE')} class="bg-white text-[#F59E0B] font-black px-4 py-2 rounded-full text-xs shadow-md active:scale-95">ฝากหิ้วเลย ➔</button>
				...
				<div class="grid grid-cols-3 gap-2.5">
					<button onclick={() => { selectedStoreId = 'store-panee'; goTo('STORE_DETAIL'); }} class="bg-white p-3 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md text-center space-y-1.5 active:scale-95 group">
						<div class="w-10 h-10 rounded-2xl bg-amber-50 text-[#F59E0B] flex items-center justify-center text-xl mx-auto group-hover:scale-110 transition-transform">🍱</div>
				...
				{#if activeOrders.length > 0}
					<button onclick={() => { currentOrderId = activeOrders[0].id; goTo('TRACKING'); }}
						class="w-full bg-white p-4 rounded-2xl border-2 border-[#F59E0B]/30 shadow-sm flex items-center justify-between hover:shadow-md transition-all active:scale-98">
						<div class="flex items-center gap-3">
							<div class="w-10 h-10 rounded-2xl bg-amber-100 text-[#F59E0B] flex items-center justify-center text-xl animate-bounce">📦</div>
							<div class="text-left">
						...
						<span class="text-[#F59E0B] font-bold text-xs">ดู ➔</span>
					</button>
				{/if}
				...
				<div class="space-y-3">
					<div class="flex justify-between items-center">
						...
						<button onclick={() => goTo('STORES')} class="text-xs text-[#F59E0B] font-bold">ดูทั้งหมด ➔</button>
					</div>
						...
						{#each MOCK_STORES.slice(0, 3) as store}
							<button onclick={() => { selectedStoreId = store.id; goTo('STORE_DETAIL'); }} class="min-w-[200px] bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden text-left group active:scale-98">
								<div class="relative h-24 overflow-hidden">
									...
									{#if store.dealText}
										<div class="absolute top-2 left-2 bg-[#F59E0B] text-white text-[9px] font-extrabold px-2 py-0.5 rounded-full">🔥 โปร!</div>
									{/if}
								</div>
		...
		{:else if screen === 'STORES'}
			<div class="p-5 space-y-4">
				...
				<!-- Search -->
				<input type="text" bind:value={storeSearchQuery} placeholder="🔍 ค้นหาร้านอาหาร เมนู..." class="w-full p-3 bg-white border border-slate-200 rounded-2xl text-xs outline-none focus:ring-2 focus:ring-[#F59E0B] shadow-sm" />

				<!-- Zone Filters -->
				<div class="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
					{#each [{ id: 'all', label: 'ทั้งหมด' }, { id: 'canteen-male', label: 'โรงชาย' }, { id: 'green-canteen', label: 'Green Canteen' }] as zone}
						<button onclick={() => (storeFilterZone = zone.id)} class={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap ${storeFilterZone === zone.id ? 'bg-[#F59E0B] text-white shadow-sm' : 'bg-white text-slate-700 border border-slate-200'}`}>{zone.label}</button>
					{/each}
				</div>
					...
					{#each filteredStores() as store}
						<button onclick={() => { selectedStoreId = store.id; goTo('STORE_DETAIL'); }} class="w-full bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden text-left group active:scale-98 hover:shadow-md transition-all">
							<div class="relative h-36 overflow-hidden">
								<img src={store.imageUrl} alt={store.name} class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
								{#if store.dealText}<div class="absolute top-3 left-3 bg-[#F59E0B] text-white text-[10px] font-extrabold px-2.5 py-1 rounded-full shadow-md">🔥 โปร</div>{/if}
							...
							<div class="p-4 space-y-1">
								...
								{#if store.dealText}<p class="text-[11px] text-[#F59E0B] font-bold">{store.dealText}</p>{/if}
							</div>
						</button>
					{/each}
				</div>
		...
		{:else if screen === 'STORE_DETAIL'}
			<div class="space-y-4 pb-24">
				...
				<div class="px-5 space-y-2">
					...
					<div class="text-xs text-slate-500">⭐ <strong class="text-slate-800">{selectedStore.rating}</strong> ({selectedStore.reviewsCount} รีวิว) • คิว: <span class="text-[#F59E0B] font-bold">{selectedStore.queueStatus}</span></div>
					{#if selectedStore.dealText}
						<div class="bg-[#FFFBEB] border border-[#F59E0B]/30 p-3 rounded-2xl text-xs text-[#F59E0B] font-medium flex items-center gap-2">
						...
						<div class="bg-white p-3.5 rounded-3xl border border-slate-100 shadow-sm flex gap-3 items-center">
							...
							<div class="flex-1 space-y-1">
								...
								<div class="flex items-center gap-2 pt-1">
									<span class="font-black text-[#F59E0B] text-sm">{item.price} ฿</span>
							...
							<div class="flex flex-col items-center">
								{#if qty > 0}
									<div class="flex items-center gap-1.5 bg-slate-100 p-1 rounded-xl">
										...
										<button onclick={() => addToCart(item, selectedStore)} class="w-6 h-6 rounded-lg bg-[#F59E0B] text-white font-bold text-xs shadow-sm">+</button>
									</div>
								{:else}
									<button onclick={() => addToCart(item, selectedStore)} class="w-9 h-9 rounded-2xl bg-[#F59E0B] text-white font-bold text-lg flex items-center justify-center shadow-md active:scale-90">+</button>
								{/if}
							</div>
						</div>
					{/each}
				...
				{#if cartTotalItems > 0}
					<div class="fixed bottom-20 left-4 right-4 max-w-md mx-auto z-30">
						<button onclick={() => goTo('CHECKOUT')} class="w-full bg-[#F59E0B] text-white p-4 rounded-3xl shadow-2xl flex items-center justify-between active:scale-98 border border-white/20">
							<div class="flex items-center gap-2">
			...
			<div class="p-5 space-y-5">
				...
				<div class="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm space-y-4">
					...
					<div class="space-y-1">
						...
						<button onclick={() => (showLocationModal = true)} class="w-full p-3 bg-[#FFFBEB] border border-[#F59E0B]/30 rounded-2xl text-xs font-bold text-slate-800 flex items-center justify-between">
							<span>📍 {currentDropoff}</span>
							<span class="text-[#F59E0B] font-extrabold">เปลี่ยน</span>
						</button>
					</div>
				</div>
				...
				<div class="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm space-y-2 text-xs">
					...
					<div class="flex justify-between"><span class="text-slate-600">ค่าหิ้วน้ำใจเพื่อน</span><span class="font-bold text-[#F59E0B]">20 ฿</span></div>
					<hr class="border-dashed border-slate-200" />
					<div class="flex justify-between items-center text-sm font-bold pt-1">
						<span class="text-slate-900">ยอดรวม</span>
						<span class="text-[#F59E0B] text-xl font-black">{customPrice + 20} ฿</span>
					</div>
				</div>

				<button onclick={submitCustomOrder} class="w-full bg-[#F59E0B] text-white font-extrabold py-4 rounded-full shadow-lg shadow-[#F59E0B]/30 text-sm active:scale-95">
			...
			<div class="p-5 space-y-5">
				...
				<div class="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm space-y-3">
					<div class="flex items-center justify-between text-xs font-bold text-slate-800">
						<span class="text-[#F59E0B]">📍 จุดส่งมอบ</span>
						<button onclick={() => (showLocationModal = true)} class="text-[#F59E0B] text-xs">เปลี่ยน ▾</button>
					</div>
				...
				<div class="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm space-y-3">
					...
					{/each}
					<button onclick={goBack} class="text-[#F59E0B] text-xs font-bold">+ เพิ่มรายการอาหาร</button>
				</div>
				...
				<div class="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm space-y-3">
					...
					<div class="flex gap-2">
						...
						<button onclick={applyPromo} class="bg-[#F59E0B] text-white px-4 py-3 rounded-2xl text-xs font-bold active:scale-95 shadow-md">ใช้โค้ด</button>
					</div>
					{#if appliedPromo}
				...
				<div class="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm space-y-2 text-xs">
					...
					<div class="flex justify-between items-center text-sm font-bold pt-1">
						...
						<span class="text-[#F59E0B] text-2xl font-black">{netTotal} ฿</span>
					</div>
				</div>
				...
				<div class="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm space-y-3">
					...
					{#each [{ val: 'PROMPTPAY' as const, label: 'PromptPay QR Code', sub: '(แนะนำ / ปลดล็อกเงินอัตโนมัติ)' }, { val: 'CASH' as const, label: 'เงินสดปลายทาง', sub: '(จ่ายให้เพื่อนตอนรับของ)' }] as pm}
						<button onclick={() => (paymentMethod = pm.val)} class={`w-full p-3.5 rounded-2xl border text-left flex items-start gap-3 transition-all ${paymentMethod === pm.val ? 'border-[#F59E0B] bg-[#FFFBEB]' : 'border-slate-200'}`}>
							<div class={`w-4 h-4 rounded-full border-2 mt-0.5 flex items-center justify-center ${paymentMethod === pm.val ? 'border-[#F59E0B] bg-[#F59E0B]' : 'border-slate-300'}`}>
				...
				<button onclick={handlePlaceOrder} class="w-full bg-[#F59E0B] text-white font-extrabold py-4 rounded-full shadow-lg shadow-[#F59E0B]/30 text-sm active:scale-95">
		...
		{:else if screen === 'TRACKING' && currentOrder}
			<div class="p-5 space-y-5">
				<div class="flex items-center justify-between">
					...
					<span class="text-[10px] font-bold bg-amber-100 text-[#F59E0B] px-2.5 py-1 rounded-full whitespace-nowrap">
					...
					<div class="space-y-3 relative before:absolute before:left-3.5 before:top-3 before:bottom-3 before:w-[2px] before:bg-slate-100">
						{#each [
							...
							<div class="flex items-center justify-between text-xs relative z-10">
								<div class="flex items-center gap-3">
									<div class={`w-7 h-7 rounded-full flex items-center justify-center font-bold text-xs ${s.done ? 'bg-[#F59E0B] text-white' : s.active ? 'bg-[#F59E0B] text-white animate-pulse' : 'bg-slate-100 border-2 border-slate-200 text-slate-300'}`}>
										{s.done ? '✓' : s.active ? '●' : '○'}
									</div>
				...
				{#if currentOrder.riderName}
					<div class="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm flex items-center justify-between">
						<div class="flex items-center gap-3">
							<div class="w-12 h-12 rounded-2xl bg-[#F59E0B] text-white flex items-center justify-center font-black text-lg shadow-md">{currentOrder.riderName.charAt(0)}</div>
							<div>
						...
						<div class="flex gap-2">
							<button onclick={() => goTo('CHAT')} class="w-10 h-10 rounded-2xl bg-[#FFFBEB] text-[#F59E0B] flex items-center justify-center text-lg shadow-sm">💬</button>
							<button onclick={() => showToast('📞 กำลังโทรออก...')} class="w-10 h-10 rounded-2xl bg-[#FFFBEB] text-[#F59E0B] flex items-center justify-center text-lg shadow-sm">📞</button>
						</div>
					</div>
				{:else}
				...
				{#if currentOrder.status === 'DELIVERING' || currentOrder.status === 'ACCEPTED'}
					<div class="bg-gradient-to-b from-[#FFFBEB] to-amber-50/80 p-6 rounded-3xl border-2 border-dashed border-[#F59E0B] text-center space-y-3 shadow-sm">
						<div class="w-10 h-10 rounded-full bg-white text-[#F59E0B] flex items-center justify-center text-xl mx-auto shadow-sm">🔑</div>
						<h3 class="font-extrabold text-slate-900 text-base">รหัส OTP ปิดงาน</h3>
						<div class="flex justify-center gap-3">
							{#each currentOrder.otpCode.split('') as digit}
								<div class="w-14 h-16 rounded-2xl bg-white border-2 border-orange-200 flex items-center justify-center font-black text-3xl text-[#F59E0B] shadow-md">{digit}</div>
							{/each}
						</div>
				...
				<div class="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm text-xs space-y-2">
					...
					<hr class="border-slate-100" />
					<div class="flex justify-between"><span class="text-slate-500">รวมชำระ</span><span class="font-black text-[#F59E0B]">{currentOrder.totalPrice} ฿</span></div>
				</div>
		...
		{:else if screen === 'CHAT'}
			<div class="flex flex-col h-[calc(100vh-120px)]">
					...
					{#each chatMessages as msg}
						...
						{:else}
							<div class="flex flex-col items-end max-w-[85%] ml-auto">
								<div class="bg-[#F59E0B] text-white p-3 rounded-2xl rounded-tr-none text-xs shadow-md">{msg.text}</div>
								<span class="text-[10px] text-slate-400 mt-0.5">{msg.time}</span>
							</div>
						{/if}
					{/each}
				...
				<div class="p-3 bg-white border-t border-slate-100 flex items-center gap-2">
					<input type="text" bind:value={chatInput} placeholder="พิมพ์ข้อความ..." onkeydown={(e) => e.key === 'Enter' && sendChat()} class="flex-1 bg-slate-100 p-3 rounded-full text-xs outline-none focus:ring-2 focus:ring-[#F59E0B]" />
					<button onclick={sendChat} class="w-10 h-10 rounded-full bg-[#F59E0B] text-white flex items-center justify-center font-bold shadow-md active:scale-90">➔</button>
				</div>
			</div>
		...
		{:else if screen === 'SUCCESS'}
			<div class="p-6 text-center space-y-5 pt-6">
				<div class="relative w-28 h-28 mx-auto">
					<div class="w-24 h-24 rounded-full bg-[#FFFBEB] text-5xl flex items-center justify-center mx-auto shadow-md border-2 border-[#F59E0B]/30">🪿</div>
					...
					<div class="flex flex-wrap justify-center gap-2">
						{#each ['ส่งไวมาก ⚡', 'อาหารยังร้อน 🍲', 'พูดจาสุภาพ 😊', 'ตรงเวลา ⏰'] as tag}
							<button onclick={() => { ratingTags = ratingTags.includes(tag) ? ratingTags.filter(t => t !== tag) : [...ratingTags, tag]; }}
								class={`px-3 py-1.5 rounded-full text-xs font-bold border ${ratingTags.includes(tag) ? 'bg-[#FFFBEB] text-[#F59E0B] border-[#F59E0B]/30' : 'bg-slate-50 text-slate-500 border-slate-200'}`}>{tag}</button>
						{/each}
					</div>
				</div>
				...
				<div class="space-y-2">
					...
					<div class="grid grid-cols-3 gap-2">
						{#each [5, 10, null] as tip}
							<button onclick={() => (extraTip = tip)} class={`py-3 rounded-2xl text-xs font-bold border ${extraTip === tip ? 'bg-[#F59E0B] text-white border-[#F59E0B]' : 'bg-white border-slate-200 text-slate-700'}`}>{tip ? `+${tip} ฿` : 'ไม่ใส่'}</button>
						{/each}
					</div>
				</div>

				<button onclick={() => { screen = 'HOME'; screenHistory = []; showToast('ขอบคุณสำหรับรีวิว! กลับสู่หน้าหลัก'); }}
					class="w-full bg-[#F59E0B] text-white font-extrabold py-4 rounded-full shadow-lg shadow-[#F59E0B]/30 text-sm active:scale-95">เสร็จสิ้น กลับหน้าหลัก</button>
			</div>
			...
			<div class="p-5 space-y-4">
				...
				<div class="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
					{#each [{ id: 'all' as const, label: 'ทั้งหมด' }, { id: 'DELIVERING' as const, label: '🏃 กำลังส่ง' }, { id: 'COMPLETED' as const, label: '✓ สำเร็จ' }] as f}
						<button onclick={() => (orderFilterStatus = f.id)} class={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap ${orderFilterStatus === f.id ? 'bg-[#F59E0B] text-white' : 'bg-white text-slate-700 border border-slate-200'}`}>{f.label}</button>
					{/each}
				</div>
				...
				{#if allFilteredOrders.length === 0}
					<div class="text-center py-10 text-slate-400">
						...
						<button onclick={() => goTo('STORES')} class="mt-3 text-[#F59E0B] font-bold text-xs">ไปสั่งอาหารเลย ➔</button>
					</div>
				{:else}
					<div class="space-y-3">
						{#each allFilteredOrders as order}
							<button onclick={() => { currentOrderId = order.id; goTo('TRACKING'); }}
								...
								<div class="flex justify-between items-center">
									...
									<span class={`text-[10px] font-bold px-2.5 py-0.5 rounded-full ${order.status === 'COMPLETED' ? 'bg-emerald-50 text-emerald-700' : order.status === 'CANCELLED' ? 'bg-red-50 text-red-600' : 'bg-amber-50 text-[#F59E0B]'}`}>
		...
		{:else if screen === 'PROFILE'}
			<div class="p-5 space-y-5">
				...
				<div class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm text-center space-y-3">
					<div class="w-20 h-20 rounded-full bg-[#FFFBEB] text-4xl flex items-center justify-center mx-auto shadow-md border-2 border-[#F59E0B]/30">🦢</div>
					<div>
				...
				<div class="grid grid-cols-3 gap-2.5">
					<div class="bg-white p-3 rounded-2xl border border-slate-100 shadow-sm text-center">
						<div class="text-lg font-black text-[#F59E0B]">{orderHistory.length}</div>
						<div class="text-[10px] text-slate-500 font-medium">ออเดอร์สำเร็จ</div>
					</div>
					<div class="bg-white p-3 rounded-2xl border border-slate-100 shadow-sm text-center">
						<div class="text-lg font-black text-[#F59E0B]">{orderHistory.reduce((s, o) => s + (o.totalPrice || 0), 0)}</div>
	...
	{#if screen !== 'CHAT'}
		<nav class="sticky bottom-0 z-40 bg-white/95 backdrop-blur-lg border-t border-slate-100 px-3 py-2 flex justify-around items-center shadow-lg">
			<button onclick={() => { screen = 'HOME'; screenHistory = []; }} class={`flex flex-col items-center gap-0.5 text-xs font-bold transition-all ${activeTab === 'home' ? 'text-[#F59E0B] scale-105' : 'text-slate-400'}`}>
				<span class="text-lg">🏠</span><span class="text-[10px]">หน้าแรก</span>
			</button>
			<button onclick={() => { screen = 'STORES'; screenHistory = ['HOME']; }} class={`flex flex-col items-center gap-0.5 text-xs font-bold transition-all relative ${activeTab === 'stores' ? 'text-[#F59E0B] scale-105' : 'text-slate-400'}`}>
				<span class="text-lg">🏬</span><span class="text-[10px]">ร้านค้า</span>
				{#if cartTotalItems > 0}
					<span class="absolute -top-1 right-0 w-4 h-4 rounded-full bg-[#F59E0B] text-white text-[9px] font-black flex items-center justify-center border border-white">{cartTotalItems}</span>
				{/if}
			</button>
			<button onclick={() => { screen = 'ORDERS'; screenHistory = ['HOME']; }} class={`flex flex-col items-center gap-0.5 text-xs font-bold transition-all relative ${activeTab === 'orders' ? 'text-[#F59E0B] scale-105' : 'text-slate-400'}`}>
				<span class="text-lg">📦</span><span class="text-[10px]">คำสั่งซื้อ</span>
				{#if activeOrders.length > 0}
					<span class="absolute -top-1 right-0 w-4 h-4 rounded-full bg-[#F59E0B] text-white text-[9px] font-black flex items-center justify-center border border-white">{activeOrders.length}</span>
				{/if}
			</button>
			<button onclick={() => { screen = 'PROFILE'; screenHistory = ['HOME']; }} class={`flex flex-col items-center gap-0.5 text-xs font-bold transition-all ${activeTab === 'profile' ? 'text-[#F59E0B] scale-105' : 'text-slate-400'}`}>
