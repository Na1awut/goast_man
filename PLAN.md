...
| 1.10 | ~~**วิธีจ่ายเงินช่วงทดลอง**~~ ✅ เสร็จ | ซ่อน PromptPay ในโหมดจริง (เงินสดอย่างเดียว) จะกลับมาเองเมื่อตั้ง `PUBLIC_PROMPTPAY_API_URL` ใน Vercel | - | S | ผู้ใช้ไม่เข้าใจผิดว่าจ่ายผ่าน QR จำลองได้ |
...
| # | งาน | รายละเอียด | ต้องการ | ขนาด | เสร็จเมื่อ |
|---|---|---|---|---|---|
| 3.1 | ~~**เลือกวิธีรับเงิน**~~ ✅ | PromptPay เข้าบัญชีทีม + ตรวจสลิปด้วย **SlipOK** | - | - | - |
| 3.2 | **QR จริง + ตรวจสลิปอัตโนมัติ** 🟡 โค้ดเสร็จ, deploy แล้ว | QR PromptPay จริงตามยอดออเดอร์, แนบสลิป, Edge Function `verify-slip` ตรวจกับ SlipOK · **เหลือ:** ใส่ secret `SLIPOK_API_KEY`/`SLIPOK_BRANCH_ID` และ `PUBLIC_PROMPTPAY_API_URL`/`PUBLIC_PROMPTPAY_ID` ใน Vercel แล้วทดลองโอนจริง 1 ครั้ง | เลข PromptPay ทีม, SlipOK key | S | โอนจริงแล้วออเดอร์ขึ้นให้คนหิ้วเอง |
| 3.3 | **เงินพักจนส่งสำเร็จ และคืนเงินเมื่อยกเลิก** 🟡 | ✅ เงินเข้าบัญชีทีม, หลัง OTP ระบบบอกยอดที่ต้องโอนให้คนหิ้ว (`rider_payouts_due()`) ทีมโอนเองแล้วกด `mark_payout_paid()` · **เหลือ:** คืนเงินเมื่อยกเลิกหลังจ่าย (ตอนนี้ทีมคืนเอง), ออเดอร์ PromptPay ที่ไม่จ่ายค้างอยู่ (ยังไม่หมดอายุเอง) | 3.2 | M | ยกเลิกแล้วได้เงินคืน, ส่งสำเร็จแล้วคนหิ้วได้เงิน |
| 3.4 | **สรุปรายได้คนหิ้ว** 🟡 | ✅ ทีมดูรายการที่ต้องโอนได้ใน SQL Editor · **เหลือ:** หน้าให้คนหิ้วเห็นรายได้ของตัวเอง, **ทิป** (ตอนนี้ผู้ซื้อกดให้ทิปได้แต่ยังไม่มีทางจ่ายทิปเข้ามา จึงยังไม่รวมในยอดโอน) | 3.3 | M | คนหิ้วเห็นรายได้ ทีมจ่ายเงินได้ถูกต้อง |
...
| D4 | ~~**วิธีรับเงินจริง**~~ ✅ ตัดสินแล้ว | **ข.** PromptPay เข้าบัญชีทีม + ตรวจสลิปด้วย SlipOK · หลัง OTP ทีมโอนให้คนหิ้วตามยอดในระบบ | - |
